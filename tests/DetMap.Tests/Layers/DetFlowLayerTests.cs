...
public class DetFlowLayerTests
{
    [Fact]
    public void Default_AllDirectionsAreBlocked()
    {
        var ff = new DetFlowLayer("flow", 8, 8);
        for (int y = 0; y < 8; y++)
        for (int x = 0; x < 8; x++)
            Assert.Equal(DetFlowLayer.Blocked, ff.Get(x, y));
    }
    ...
    public void Reset_RestoresAllToBlocked()
    {
        ...
        ff.Reset();
        Assert.Equal(DetFlowLayer.Blocked, ff.Get(0, 0));
        Assert.Equal(DetFlowLayer.Blocked, ff.Get(3, 3));
    }

    [Fact]
    public void AllEightDirections_StoredCorrectly()
    {
        var ff = new DetFlowLayer("flow", 10, 10);
    ...
    {
        Assert.Equal(255, DetFlowLayer.Blocked);
    }
